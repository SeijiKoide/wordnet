...
;;;
;;; WordNet Definition that is based on W3C Working Draft 19 June 2006,
    ...
    (loop for entry = (read-line data-stream nil nil)
        ...
        do (destructuring-bind
             ...
             (let ((ss_type (ss_type-from-symbol p1)))
               ...
               (with-list-split-after (* 2 num-senses)
                 (sense-stuff pointers-and-more) senses-and-more
                 ;; abstraction 0 abstract_entity 0
                 (declare (ignore pointers-and-more))
                 (format t "~%Reading ~S ... " (if (eq pos :adjective) (get-adj-word (car sense-stuff)) (car sense-stuff)))
                 (loop for (word nil) on sense-stuff by #'cddr with word-name
                     do (setq word (if (eq pos :adjective) (get-adj-word word) word))
             ...
             (let ((ss_type (ss_type-from-symbol p1)))
               ...
               (with-list-split-after (* 2 num-senses)
                 ...
                 (let ((primary-name
                       ...
                       (primary-lex_id (second sense-stuff)))
                   (declare (ignore primary-lex_id))
                     ...
                     (let ((word&frame-list nil)
                       ...
                       (let ((synnumber
                         ...
                         (let ((word-name (make-word-name primary-name))
                               ...
                               (target-word-sense-names
                                (loop for (word nil) on (cddr sense-stuff) by #'cddr
                                    do (when (eq pos :adjective)
                           ...
                           (format outstream
                             ...
                             (+ offset
                                (ecase pos
                                  ...
                                  (:adjective 300000000)
                                  (:adverb 400000000))))
                           (loop for sense-name in (sort (copy-list (cons source-word-sense-name target-word-sense-names))
                                                         #'string<)
                           ...
                           (loop for (word nil) on (cddr sense-stuff) by #'cddr
                               for sense-name in target-word-sense-names
                               do
                                 (when (eq pos :adjective)
                                   (setq word (get-adj-word word)))
...
(defun make-one-big-rdf-file-noun ()
  (setq *outputted-word-list* nil)
  (with-open-file (outstream *one-big-rdf-file-noun-path* :direction :output)
    (format outstream *xml-decl*)
    (format outstream *doc-decl*)
    (format outstream *rdf-decl-prolog*)
    (%make-one-big-rdf-file :noun outstream)
    (format outstream *rdf-decl-epilog*)
    t))

(defun make-one-big-rdf-file-verb ()
  (setq *outputted-word-list* nil)
  (with-open-file (outstream *one-big-rdf-file-verb-path* :direction :output)
    (format outstream *xml-decl*)
    (format outstream *doc-decl*)
    (format outstream *rdf-decl-prolog*)
    (%make-one-big-rdf-file :verb outstream)
    (format outstream *rdf-decl-epilog*)
    t))

(defun make-one-big-rdf-file-adjective ()
  (setq *outputted-word-list* nil)
  (with-open-file (outstream *one-big-rdf-file-adjective-path* :direction :output)
    (format outstream *xml-decl*)
    (format outstream *doc-decl*)
    (format outstream *rdf-decl-prolog*)
    (%make-one-big-rdf-file :adjective outstream)
    (format outstream *rdf-decl-epilog*)
    t))

(defun make-one-big-rdf-file-adverb ()
  (setq *outputted-word-list* nil)
  (with-open-file (outstream *one-big-rdf-file-adverb-path* :direction :output)
    (format outstream *xml-decl*)
    (format outstream *doc-decl*)
    (format outstream *rdf-decl-prolog*)
    (%make-one-big-rdf-file :adverb outstream)
    (format outstream *rdf-decl-epilog*)
    t))

(defun %make-one-big-rdf-file (pos outstream)
  (with-open-file (data-stream (pos-data-file pos) :direction :input)
    (loop for entry = (read-line data-stream nil nil)
        while entry
        unless (char= #\Space (char entry 0))
                     ...
                     (let ((word&frame-list nil)
                       ...
                       (let ((synnumber
                         ...
                         (let ((word-name (make-word-name primary-name))
                           ...
                           (unless (member word-name *outputted-word-list*)
                             (let ((sense-names
                                    (loop for offset in (offsets-in-index-entry (string-downcase primary-name) pos)
                                        with i = 0
                                        collect (make-sense-name primary-name ss_type (incf i)))))
                               (format outstream
                                   (if (collocated-p primary-name) *collocation-description-template*
                                     *word-description-template*)
                                 word-name (underscore2space primary-name) sense-names)
                               (push word-name *outputted-word-list*)))
                           ;; First word sense output
                           (format outstream
                               (ecase ss_type
                           ...
                           (when hypernym-instance-target-synset-names
                             (format outstream
                                 (ecase pos
                                   (:noun "~{  <wn21schema:hypernymInstanceOf rdf:resource=\"&wn30instances;~A\"/>~^~%~}~%")
                                   (:verb "~{  <wn21schema:hypernymInstanceOf rdf:resource=\"&wn30instances;~A\"/>~^~%~}~%"))
                               (sort hypernym-instance-target-synset-names #'string<)))
                           (when membermeronym-target-synset-names
                             (format outstream
                                 (ecase ss_type
